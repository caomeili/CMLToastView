...
#import "ToastSupport.h"
#import "ToastDefaultView.h"
...
-(void)dissmissWithDelayTime:(NSTimeInterval)time {
    [NSTimer scheduledTimerWithTimeInterval:time target:self selector:@selector(removeToastTimerAction) userInfo:nil repeats:NO];
}
- (void)removeToastTimerAction {
    [self removeToast];
}
- (void)removeToast {
    if (_completionBlock) {
...
- (ToastSupport *)showInTime:(NSTimeInterval)time {
    [self show];
...
@implementation ToastSupportFactory

+(instancetype)sharedInstance {
    static ToastSupportFactory *toastInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        // 自定义view
        toastInstance = [[ToastSupportFactory alloc] init];
...
- (void)setupIndicatorView {
    ...
    _indicatorView = [[ToastDefaultView alloc] initWithFrame:CGRectMake(0, 0, 40, 40)];
    // Update styling
    ToastDefaultView *indefiniteAnimatedView = (ToastDefaultView*)_indicatorView;
...
- (ToastSupport*)createLoadingWithMessage:(NSString *)msg detial:(NSString *)detail context:(void *)context {
    ToastSupport *support = [[ToastSupport alloc] init];
    if (customViewClass) {
        UIView<CustomToastViewDelegate> *customView = [[customViewClass alloc] init];
...
- (ToastSupport *)createImageWithImageName:(NSString *)imageName message:(NSString *)msg success:(BOOL)success context:(void *)context{
    ToastSupport *support = [[ToastSupport alloc] init];
    if (success == YES) {
        if (customViewClass) {
            UIView<CustomToastViewDelegate> *customView = [[customViewClass alloc] init];
    ...
    _indicatorView = nil;
    [self setupUI];
    NSBundle *bundle = [NSBundle bundleForClass:[ToastSupportFactory class]];
    NSURL *url = [bundle URLForResource:@"CMLToastView" withExtension:@"bundle"];
